(function () {
  ...
  // 这里在解析阶段就给 <html> 打标记，配合 CSS 第一帧就把它藏掉（回到探索页会自动解除）。
  (function () {
    try {
      var p0 = window.location.pathname;
      var base0 = '/api/ext/tencent-channel/panel/';
      var tail0 = p0.indexOf(base0) === 0 ? p0.slice(base0.length) : p0.replace(/^\/+/, '');
      tail0 = tail0.replace(/^\/+/, '').replace(/\/+$/, '');
      if (tail0 && tail0 !== 'explore' && tail0.indexOf('index') !== 0) {
        document.documentElement.className += ' txpd-deep-route';
      }
    } catch (e) { /* 忽略 */ }
  })();
  ...
  var _entryPath = '';   // 入口要恢复的深层路径（渲染出来之前一直盯着，见 syncSsrExploreVisibility）
  function _tailOf(pathname) {
    var p = pathname.indexOf(PANEL_BASE) === 0 ? pathname.slice(PANEL_BASE.length) : pathname.replace(/^\/+/, '');
  ...
  try {
    ...
    if (!TXPD_EMBED && loc.pathname.indexOf(PANEL_BASE) !== 0) {
      ...
      var targetPath = savedPath ? String(savedPath).replace(/^\/+/, '').replace(/\/+$/, '') : 'explore';
      if (targetPath === 'explore' || !targetPath) {
        window.history.replaceState(window.history.state, '', PANEL_BASE + 'explore');
      } else {
        // 原样带过去会被 vue-router 当成初始路由，于是地址又被拉回探索页。
        _entryPath = targetPath;
        try {
          var pl = document.querySelector('script[data-nuxt-data="nuxt-app"]');
          if (pl && pl.parentNode) pl.parentNode.removeChild(pl);
        } catch (e2) { /* 忽略 */ }
        try {
          window.history.replaceState(null, '', PANEL_BASE + targetPath);
        } catch (e2) { /* 极少数浏览器不允许改地址：保持原地址，交给应用自己决定 */ }
        // 之后换 replaceState 不再堆历史条目），目标页渲染完成（探索页节点消失）才收工。
        (function watchEntry(ticks) {
          setTimeout(function () {
            try {
              if (!_entryPath) return;
              var tail = _tailOf(window.location.pathname);
              // 目标页出来了：地址对上且探索页节点已经不在了
              if (tail === _entryPath && !document.getElementById('explorePage')) { _entryPath = ''; return; }
              // 应用执意停在探索页（无权限之类）→ 放手，别一直挡着
              if (ticks >= 8 && tail === 'explore' && document.getElementById('explorePage')) { _entryPath = ''; return; }
              if (ticks >= 32) { _entryPath = ''; return; }   // 8s 还没成：交给应用
              var target = PANEL_BASE + _entryPath;
              if (tail !== _entryPath) {
                if (ticks === 0) window.history.pushState(null, '', target);
  ...
  // 都会连打这几条 console.error 刷屏。这里只压这几条已知噪音，其它报错照常输出。
  (function () {
    var pats = ['[turingSdk]', 'turingSDK', 'initTuringSdk', 'deviceToken2SignToken', 'activate sign token'];
    ['error', 'warn'].forEach(function (level) {
      var orig = console[level];
      if (typeof orig !== 'function') return;
      console[level] = function () {
        try {
          var first = String(arguments.length ? arguments[0] : '');
          for (var i = 0; i < pats.length; i++) {
            if (first.indexOf(pats[i]) !== -1) return;
          }
        } catch (e) { /* 判断失败就照常输出 */ }
  ...
  function renderAccountList() {
    ...
    api('/accounts').then(function (r) {
      ...
      accs.forEach(function (a) {
        ...
        // 每个槽位都能删（包括当前槽位：后端会把 current 切到剩下的槽位）
        var del = el('span', { 'class': 'txpd-acct-del' }, '删除');
        del.addEventListener('click', function (e) {
          e.stopPropagation();
          var tip = a.current ? '（这是当前槽位，删掉后会自动切到其它槽位）' : '';
          if (confirm('确定删除账号「' + (a.nickname || a.name) + '」？' + tip)) {
            api('/accounts/delete', { method: 'POST', body: { name: a.name } }).then(function (r2) {
              if (r2 && r2.success === false) { alert(r2.message || '删除失败'); return; }
              invalidateAccounts();
              invalidateGuilds();
              renderAccountList();
            }).catch(function (err) { alert('删除失败：' + String((err && err.message) || err)); });
          }
        });
  ...
  function renderQrStage() {
    ...
    // 先看有没有「建了但还没登录」的槽位：有就复用它，不要每次点登录都新建
    api('/accounts').then(function (ra) {
      var accs = (ra && ra.data && ra.data.accounts) || [];
      var fresh = null;
      for (var i = 0; i < accs.length; i++) {
        if (accs[i] && !accs[i].logged_in) { fresh = accs[i].name; break; }
      }
      return fresh;
    }).catch(function () { return null; }).then(function (reuse) {
      return api('/accounts/add', { method: 'POST', body: reuse ? { user: reuse } : {} });
    }).then(function (r) {
      if (!r.success) {
        stage.innerHTML = '';
  ...
  style.textContent = [
    ...
    // 深路径整页加载：探索页 SSR 先闪一下 → 直接藏掉
    'html.txpd-deep-route #explorePage{display:none!important;}',
    ...
    '.txpd-avatar-ph{display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#5b8cff,#8a6cff);color:#fff;font-size:14px;font-weight:600;border-radius:50%;}',
    // 交互顺滑化：过渡/进入动画（尊重系统「减少动态效果」）----------
    '.txpd-mg-btn,.txpd-join-btn,.app-menu-list .menu-item{transition:background-color .16s ease,opacity .16s ease,transform .12s ease,color .16s ease;}',
    '.txpd-mg-btn:active,.txpd-join-btn:active{transform:scale(.97);}',
    ...
    + '.txpd-manage,#txpd-drawer,#txpd-acct-mask,#txpd-acct-dlg{animation:none!important;}'
    + '.txpd-mg-btn,.txpd-join-btn,.app-menu-list .menu-item{transition:none!important;}'
    + '}',
    // 缺登录态时的整页提示
  ...
  setInterval(function () {
    getAccountsState().then(function (any) {
      ...
      loadJoinedGuilds().then(function () {
        ...
        ensureNavEntries();
        syncPublishArea();
        refreshJoinedAvatars();   // 官方「我的频道」晚一步渲染：定期回来把头补齐
        syncAsideTitles();        // 分区标题：网页账号 / CLI 账号写清楚（会被应用重渲染还原）
        syncLikedMarks();
        ensureGuildMeta();
      ensureGuildHeaderFallback();
        ensureGuildHeaderFallback();
  ...
  // 退出频道（CLI: manage leave-guild）——高风险操作，先确认
  function openLeaveGuild(anchor) {
    var ctx = currentGuild();
    if (!ctx.g || !ctx.id) { toast('未识别当前频道（可能还没加载完频道列表）'); return; }
    var name = ctx.g.name || ctx.num || ctx.id;
    var dlgP = openPopover(anchor, '退出频道');
    dlgP.appendChild(el('div', { style: 'font-size:13px;color:#333;line-height:1.7;margin-bottom:10px;' },
      '确定让插件账号（CLI）退出「' + name + '」吗？退出后将无法以该账号在该频道发帖、评论、管理。'));
    var row = el('div', { style: 'display:flex;gap:8px;align-items:center;' });
    var ok = primaryBtn('确认退出');
    ok.style.background = '#e5484d';
    var cancel = el('button', { type: 'button', style: 'border:1px solid rgba(219,220,224,.9);background:#fff;border-radius:100px;height:34px;padding:0 16px;font-size:13px;color:#666;cursor:pointer;font-family:inherit;' }, '取消');
    var status = statusLine();
    row.appendChild(ok);
    row.appendChild(cancel);
    dlgP.appendChild(row);
    ...
    ok.addEventListener('click', function () {
      ...
      api('/cli', { method: 'POST', body: { action: 'leave-guild', params: { guild_id: ctx.id } } }).then(function (r) {
        if (!r.success) throw new Error(r.message || '退出失败');
        status.style.color = '#15a361';
        status.textContent = '✓ 已退出「' + name + '」';
        invalidateGuilds();
        loadJoinedGuilds().then(function () {
          try { queueUiSync(); } catch (e) { /* 忽略 */ }
          if (typeof ensureJoinedSection === 'function') ensureJoinedSection();
        ...
        status.textContent = String((e && e.message) || e).slice(0, 200);
        ok.disabled = false;
      });
    });
  }
  ...
  var JOIN_TYPE_LABELS = {
    ...
    JOIN_GUILD_TYPE_MULTI_QUESTION: '多题验证',
    JOIN_GUILD_TYPE_MULTI_ANSWER_WITH_ADMIN_AUDIT: '多题验证',
    JOIN_GUILD_TYPE_QUIZ: '测试题',
  };
  function joinTypeLabel(t) {
    var raw = String(t || '').trim();
    if (!raw) return '';
    var compact = raw.replace(/_/g, '').toUpperCase();
    for (var k in JOIN_TYPE_LABELS) {
      if (k === raw || k.replace(/_/g, '').toUpperCase() === compact) return JOIN_TYPE_LABELS[k];
    }
    return raw;
  }
  //   DISABLE                             禁止加入
  ...
  function joinQuestionItems(setting) {
    var box = (setting && (setting.question || setting.questions)) || {};
    var items = box.items || box.question || box.list || [];
    if (!Array.isArray(items)) items = [];
    return items.map(function (it) {
      if (typeof it === 'string') return { title: it, options: [] };
      var title = (it && (it.title || it.question || it.content || it.text)) || '';
      var opts = (it && (it.answers || it.options || it.answerList)) || [];
      if (!Array.isArray(opts)) opts = [];
      var options = opts.map(function (o) {
        if (typeof o === 'string') return o;
        return (o && (o.content || o.title || o.text || o.answer)) || '';
      }).filter(function (x) { return !!x; });
      return { title: String(title || ''), options: options };
    }).filter(function (x) { return x.title || x.options.length; });
  }
  // 「加入」回复里是否在要附言 / 答题（CLI 用 need_verification，也会直接在文案里说）
  ...
  function openJoinFlow(guildNumber) {
    ...
    api('/cli', { method: 'POST', body: { action: 'search-guild', params: { keyword: guildNumber, scope: 'channel' } } }).then(function (r) {
      ...
      if (!r.success || !ch) {
        fail('未找到频道「' + guildNumber + '」' + (r.message ? '：' + r.message : ''));
        return;
      }
      // 加入方式：有的频道要附言 / 答题，先问清楚再加入（免得点了没反应就"加入成功"）
      return api('/cli', { method: 'POST', body: { action: 'join-setting', params: { guild_id: ch.guild_id } } }).then(function (rs) {
        var setting = ((rs.data && rs.data.data && rs.data.data.setting) || (rs.data && rs.data.data) || {});
        var joinType = setting.joinType || setting.join_type || '';
        var questions = joinQuestionItems(setting);
        ...
        if (joinTypeLabel(joinType)) stage.appendChild(line('加入方式', joinTypeLabel(joinType)));
        ...
        if (String(joinType).replace(/_/g, '').toUpperCase().indexOf('DISABLE') !== -1) {
          joinBtn.disabled = true;
          joinBtn.textContent = '该频道禁止加入';
        }
        stage.appendChild(joinBtn);
        var status = el('div', { style: 'font-size:13px;color:#2b64f5;margin-top:8px;min-height:18px;word-break:break-all;' });
        stage.appendChild(status);
        ...
        // 用已加入列表反查一次（列表有延迟就重试），避免面板报一个假的「✓ 已加入」。
        function checkJoined(retries) {
          return loadJoinedGuilds().then(function (list) {
            var want = numVariants(ch.guild_number).concat(ch.guild_id ? [String(ch.guild_id)] : []);
            var hit = false;
            (list || []).forEach(function (g) {
              if (!g) return;
              numVariants(g.guild_number).concat(g.guild_id ? [String(g.guild_id)] : []).forEach(function (k) {
                if (k && want.indexOf(k) !== -1) hit = true;
              });
            });
            if (hit) return true;
            if (retries > 0) {
          ...
          req.then(function (r2) {
            ...
            return checkJoined(2).then(function (joined) {
              if (!joined) {
                ...
                status.style.color = '#e5484d';
                joinBtn.disabled = false;
                joinBtn.textContent = '补充验证后再试';
                toast('加入可能未生效，请补充验证后重试');
                return;
              }
              ...
              try { queueUiSync(); } catch (e) { /* 忽略 */ }
              setTimeout(ensureGuildMeta, 0);
              toast('✓ 已加入频道「' + (ch.name || guildNumber) + '」');
            });
          }).catch(function (e) {
            status.textContent = String((e && e.message) || '网络错误，请重试');
            status.style.color = '#e5484d';
            joinBtn.disabled = false;
            joinBtn.textContent = '重试加入';
          });
  ...
  function spaNavigate(path) {
    ...
    try {
      ...
      window.dispatchEvent(new PopStateEvent('popstate', { state: window.history.state }));
      // target 整页加载（用户看到的「刷新一下」）；现在只在应用内重试，绝不重载文档。
      (function retry(attempt) {
        setTimeout(function () {
          try {
            if (attempt >= 3) return;
            if (_tailOf(window.location.pathname) === _tailOf(path)) return;   // 已到位
  ...
  // 官方侧栏里同一频道的头像地址（带官方参数，能正常加载）；找不到就返回空
  function officialAvatarOf(g) {
    if (!g) return '';
    var gid = String(g.guild_id || '');
    var name = String(g.name || '').trim();
    var nameBare = name.replace(/频道$/, '').trim();
    var nums = numVariants(g.guild_number);
    if (gid) nums = nums.concat(numVariants(gid));
    var groups = ['.aside-group--my-guild', '.aside-group--my-temp-guild'];
    for (var gi = 0; gi < groups.length; gi++) {
      var items = document.querySelectorAll(groups[gi] + ' .my-guild-item');
      for (var i = 0; i < items.length; i++) {
        var it = items[i];
        var img = it.querySelector('img');
        var src = img ? (img.getAttribute('src') || '') : '';
        // 懒加载的图先给 data:/占位地址，真实地址在 data-src 上
        if (img && !/^https?:/i.test(src)) {
          src = img.getAttribute('data-src') || img.getAttribute('data-original') || src;
        }
        if (!src || !/^https?:/i.test(src)) continue;
        if (gid && src.indexOf('/' + gid + '/') !== -1) return src;
        // 频道号：官方条目的链接 / 头像地址里都带得出来（两套编码都试）
        var link = it.querySelector('a[href]');
        var href = link ? (link.getAttribute('href') || '') : '';
        var hay = href + ' ' + src + ' ' + (it.getAttribute('data-gnum') || '');
        for (var k = 0; k < nums.length; k++) {
          if (nums[k] && hay.indexOf(nums[k]) !== -1) return src;
        }
        var nameEl = it.querySelector('.item-name');
        var n = nameEl ? (nameEl.textContent || '').trim() : '';
        if (name && n && (n === name || n.replace(/频道$/, '').trim() === nameBare)) return src;
      }
    }
    return '';
  }

  // 都没有才用首字占位。拼出来的 gtimg 直链会 403/协议错误，所以只认接口给的字符串。
  var AVATAR_FIELDS = ['avatar_url', 'avatarUrl', 'avatar', 'guild_avatar', 'guild_avatar_url', 'guildAvatar', 'icon', 'guild_icon'];
  function avatarFromObject(obj) {
    if (!obj || typeof obj !== 'object') return '';
    for (var i = 0; i < AVATAR_FIELDS.length; i++) {
      var v = obj[AVATAR_FIELDS[i]];
      if (typeof v === 'string' && /^https?:\/\//i.test(v)) return v;
    }
    // 少数接口把频道信息裹在 guild_info / msg_guild_info / guildInfo 里
    var subs = ['guild_info', 'guildInfo', 'msg_guild_info', 'info'];
    for (var s = 0; s < subs.length; s++) {
      var sub = obj[subs[s]];
      if (sub && typeof sub === 'object') {
        for (var k = 0; k < AVATAR_FIELDS.length; k++) {
          var sv = sub[AVATAR_FIELDS[k]];
          if (typeof sv === 'string' && /^https?:\/\//i.test(sv)) return sv;
        }
      }
    }
    return '';
  }
  function joinedAvatarUrl(g) {
    var official = officialAvatarOf(g);
    if (official) return official;
    var fromCli = avatarFromObject(g);
    if (fromCli) return fromCli;
    // 动态页抓过的频道头像缓存（guild-info 的 avatar_url，按 guild_id 存）
    if (g) {
      var store = dynIconCache();
      var keys = [];
      if (g.guild_id) keys.push(String(g.guild_id));
      numVariants(g.guild_number).forEach(function (k) { keys.push(k); });
      if (g.guild_number) keys.push(String(g.guild_number));
      for (var i = 0; i < keys.length; i++) {
        if (store[keys[i]]) return store[keys[i]];
      }
    }
    return '';
  }
  // 把头像画进一行（已经有图就不动）
  var _avatarFailedAt = {};   // 加载失败的地址 → 时间戳：10 分钟内不再重试，免得同步循环里反复发请求
  function avatarUsable(url) {
    var t = _avatarFailedAt[url];
    return !t || (Date.now() - t) > 600000;
  }
  function paintJoinedAvatar(item, g) {
    var wrap = item.querySelector('.item-avatar-wrap');
    if (!wrap) return;
    if (wrap.querySelector('img.item-avatar')) return;
    var url = joinedAvatarUrl(g);
    if (!url || !avatarUsable(url)) return;            // 拿不到：留着占位，下次同步再试
    var img = el('img', {
      'class': 'item-avatar',
      'src': url,
      'alt': g.name || '',
    ...
    img.addEventListener('error', function () {
      // 官方那份地址也可能过期：记下失败、撤掉图，回到首字占位（10 分钟后再试）
      _avatarFailedAt[url] = Date.now();
      if (img.parentNode) img.parentNode.removeChild(img);
      if (wrap && !wrap.querySelector('.txpd-avatar-ph')) {
        wrap.appendChild(avatarPlaceholder(g.name || g.guild_number || '?'));
      }
    });
    var ph = wrap.querySelector('.txpd-avatar-ph');
    if (ph && ph.parentNode) ph.parentNode.removeChild(ph);
    wrap.appendChild(img);
  }
  // 用户看到的就是「已加入的频道不显示头像/表情」。所以每轮同步都回头补一遍。
  function refreshJoinedAvatars() {
    var items = document.querySelectorAll('.aside-group--txpd-joined .my-guild-item');
    for (var i = 0; i < items.length; i++) {
      var it = items[i];
      if (it.querySelector('img.item-avatar')) continue;
      var g = it._txpdGuild;
      if (!g) {
        var gnum = it.getAttribute('data-gnum') || '';
        var keys = numVariants(gnum);
        if (_guildsCache) {
          for (var j = 0; j < _guildsCache.length && !g; j++) {
            var cand = _guildsCache[j];
            var ck = numVariants(cand.guild_number).concat(cand.guild_id ? [String(cand.guild_id)] : []);
            for (var k = 0; k < ck.length; k++) {
              if (keys.indexOf(ck[k]) !== -1) { g = cand; break; }
            }
          }
        }
      }
      if (g) paintJoinedAvatar(it, g);
      // guild-info（结果进同一个本地缓存，每个频道只问一次）。
      if (!it.querySelector('img.item-avatar') && g) fetchSideAvatar(g, it);
    }
  }

  // 以前只能一直显示首字，直到用户点进那个频道官方条目才渲染出来。
  var _sideAvatarAsked = {};   // guild_id → 上次问的时间：同一个频道 30 分钟内只问一次
  var _sideAvatarGot = {};     // guild_id → 1：已经拿到了头像地址，不必再问
  var _sideAvatarBusy = false; // CLI 单次约 4s：同一时刻只问一个频道，其余留给下一轮同步
  function fetchSideAvatar(g, item) {
    var gid = String((g && g.guild_id) || '').trim();
    var num = String((g && g.guild_number) || '').trim();
    if (!gid) {
      // guild-info 只认 guild_id：有频道号就先换 id（转换结果也有缓存）
      if (!num) return;
      if (_sideAvatarAsked['n:' + num]) return;
      _sideAvatarAsked['n:' + num] = Date.now();
      resolveGuildIdByNumber(num, function (found) {
        if (found) fetchSideAvatar({ guild_id: found, guild_number: num, name: g.name }, item);
      });
      return;
    }
    if (_sideAvatarGot[gid]) return;
    if (_sideAvatarBusy) return;                                   // 串行：一次只问一个
    var asked = _sideAvatarAsked[gid] || 0;
    if (Date.now() - asked < 1800000) return;
    _sideAvatarAsked[gid] = Date.now();
    _sideAvatarBusy = true;
    api('/cli', { method: 'POST', body: { action: 'guild-info', params: { guild_id: gid } } }).then(function (r) {
      var d = (r.data && r.data.data) || {};
      var url = avatarFromObject(d);
      if (!url) return;
      var store = dynIconCache();
      store[gid] = url;
      dynIconSave(store);
      _sideAvatarGot[gid] = 1;
      var items = document.querySelectorAll('.aside-group--txpd-joined .my-guild-item');
      for (var i = 0; i < items.length; i++) {
        var it = items[i];
        var gg = it._txpdGuild || {};
        var same = String(gg.guild_id || '') === gid
          || (num && String(gg.guild_number || '') === num)
          || (num && (it.getAttribute('data-gnum') || '') === num);
        if (!same || it.querySelector('img.item-avatar')) continue;
        paintJoinedAvatar(it, {
          guild_id: gid, guild_number: num,
          name: gg.name || (g && g.name) || '',
          avatar_url: url,
        });
      }
    }).catch(function () { /* 失败就保持占位，30 分钟后再说 */ }).then(function () { _sideAvatarBusy = false; });
  }

  // 官方同款占位：昵称首字 + 主题色底
  function avatarPlaceholder(text) {
    var d = el('div', { 'class': 'item-avatar txpd-avatar-ph' });
    d.textContent = String(text || '?').trim().charAt(0) || '?';
    return d;
  }
  ...
  // 官方标题是 Vue 托管的文本节点：只改文本、不删节点；应用重渲染会还原，所以每轮同步补一次。
  function syncAsideTitles() {
    var groups = ['.aside-group--my-guild', '.aside-group--my-temp-guild'];
    for (var i = 0; i < groups.length; i++) {
      var titles = document.querySelectorAll(groups[i] + ' .group-title');
      for (var j = 0; j < titles.length; j++) {
        var h = titles[j];
        for (var k = 0; k < h.childNodes.length; k++) {
          var n = h.childNodes[k];
          if (n.nodeType !== 3) continue;
          var t = n.textContent || '';
          if (t.indexOf('我的频道') !== -1) n.textContent = t.replace('我的频道', '网页账号加入的频道');
        }
      }
    }
    var mine = document.querySelector('.aside-group--txpd-joined .group-title');
    if (mine && mine.textContent.indexOf('CLI账号') === -1) mine.textContent = 'CLI账号加入的频道';
  }
  ...
  function ensureJoinedSection() {
    ...
    header.appendChild(el('h3', { 'class': 'group-title' }, 'CLI账号加入的频道'));
  ...
  function stopWebLoginPoll() {
    ...
    _wlActive = false;   // 链停了就把标记也清掉：否则状态行会被永久冻结在「等待扫码…」
  }
  // 深路径时隐藏 SSR 的探索页（第一帧靠 head 里的标记 + CSS，这里负责后续切换）
  function syncSsrExploreVisibility() {
    try {
      var tail = routeKey();
      var deep = !!(tail && tail !== 'explore' && tail.indexOf('index') !== 0);
      // 否则「先进频道、闪一眼探索页、再回到频道」——用户看到的就是「刷新一下」
      if (!deep && _entryPath) deep = true;
      var cls = document.documentElement.classList;
      if (deep) cls.add('txpd-deep-route');
      else cls.remove('txpd-deep-route');
    } catch (e) { /* 忽略 */ }
  }

  function routeKey() {
    return _tailOf(window.location.pathname);
  }
  ...
  function syncManageStatus(force) {
    ...
    api('/web-login/status').then(function (r) {
      var d = (r && r.data) || {};
      // 链已经停了就必须按真实状态刷新 —— 否则会永久停在「等待扫码…」（用户实测过）
      var polling = !!(_wlActive && _wlPollTimer);
      if (polling) return;
      var st = document.getElementById('txpd-wl-status');
      if (st) {
      ...
      // 后端还在等扫码（比如页面刚刷新过）→ 把轮询接回来，别让用户干等
      if (!d.logged_in && d.polling && !_wlActive) {
        var box = document.getElementById('txpd-wl-qrbox');
        if (box && box.style.display !== 'none') pollWebLogin();
      }
    }).catch(function () { /* 忽略 */ });
    api('/accounts').then(function (r) {
  ...
  function ensureTopbarButtons() {
    ...
    var want = joined ? (isAdmin ? ['config', 'schedule', 'members', 'leave'] : ['schedule', 'members', 'leave']) : [];
    var meta = {
      config: ['频道配置', 'assets/common.svg#guildSetting'],
      schedule: ['定时发帖', null],
      members: ['成员列表', 'assets/common.svg#user'],
      leave: ['退出频道', 'assets/common.svg#withdraw'],
    };
  ...
  function ensureLoginNotice() {
    ...
    var missing = '';
    // 只要 CLI 账号在，网页没登录也照常看内容，不该再挡页面（以前会显示「网页未登录，无法查看」）
    if (need === 'web' && !TXPD_WEB_LOGGED_IN && _cliLoggedIn === false) missing = '腾讯频道网页未登录，也没有可用的频道账号（CLI）';
  ...
  function currentNavKey() {
    // 打开插件的整页视图时，高亮对应的那一项（发私信整页算在「私信列表」上）
    if (_pageOpen) {
      // 页面 key → 导航 key：插件管理页的 key 是 manage，导航项是 admin
      if (_pageOpen === 'dmSend') return 'dm';
      if (_pageOpen === 'manage') return 'admin';
      return _pageOpen;
    }
    var tail = routeKey();
    if (tail.indexOf('index') === 0) return 'dynamic';
    if (tail.indexOf('explore') === 0 || !tail) return 'explore';
    return '';
  }
  function markNavActive(mine) {
    if (!mine) {
      mine = {};
      NAV_FIXED.forEach(function (slot) {
        var n = document.querySelector('.txpd-nav-' + slot.key);
        if (n) mine[slot.key] = n;
      });
    }
    var cur = currentNavKey();
    if (cur === _navActiveKey) return;   // 状态没变就不写（避免 class 反复变动）
    _navActiveKey = cur;
  ...
  // 点击发布卡 → 展开插件自己的编辑器（发表走 CLI），从而绕开官方那条走不通的路。
  function hookPublishArea(container) {
    if (!container || container.getAttribute('data-txpd-hooked') === '1') return;
    container.setAttribute('data-txpd-hooked', '1');
    // 捕获阶段挂在容器（祖先）上：应用自己的处理器在目标元素上，捕获先于它触发
    container.addEventListener('click', function (e) {
      if (container.getAttribute('data-txpd-expanded') === '1') return;   // 已展开：交给编辑器自己
      var t = e.target;
      if (t && t.closest && t.closest('[data-txpd-editor-added]')) return;
      var emojiIcon = t && t.closest ? t.closest('.toolbar-button .icon-emoji') : null;
      e.preventDefault();
      e.stopImmediatePropagation();
      e.stopPropagation();
  ...
  function syncPublishArea() {
    var ctx = currentGuild();
    var containers = publishAreaContainers();
    if (!containers.length) return;
    containers.forEach(hookPublishArea);
    // 每轮同步补一次隐藏；幂等：已标记的不再动，我们自己追加的节点（带标记的 / file input）跳过。
    containers.forEach(function (box) {
      if (box.getAttribute('data-txpd-expanded') !== '1') return;
      Array.prototype.forEach.call(Array.prototype.slice.call(box.children), function (ch) {
        if (!ch.getAttribute) return;
        if (ch.getAttribute('data-txpd-editor-added') === '1') return;   // 我们的复刻
        if (ch.tagName === 'INPUT') return;                              // 我们的选图 input
        if (ch.getAttribute('data-txpd-hidden-editor') !== '1') {
          ch.setAttribute('data-txpd-hidden-editor', '1');
          if (ch.style) ch.style.setProperty('display', 'none', 'important');
        }
      });
    });
    var joined = !!(ctx.num && _joinedKeys && _joinedKeys[ctx.num]);
    // 收掉，用户正打字就没了，表现为点了没反应 / 发不出去）
  ...
  // 频道列表命中不了时，最多每 60 秒强制刷新一次（列表本身有 1 小时缓存）
  var _refreshJoinedTs = 0;
  function refreshJoinedSoon() {
    var now = Date.now();
    if (now - _refreshJoinedTs < 60000) return;
    _refreshJoinedTs = now;
    invalidateGuilds();
    loadJoinedGuilds().then(function () { setTimeout(function () { ensureGuildMeta(); ensureTopbarButtons(); }, 0); }).catch(function () { });
  }
  ...
  function guildMetaIdentity(num) {
    if (_guildsCache && _guildsCache.length) {
      ...
      // 并且顺手刷新一次频道列表（列表有 1 小时缓存，刚加入的频道可能还没进来）
      var off = officialMemberKeys();
      if ((num && off.gids[num]) || (pn && off.names[pn])) {
        return { role: '已加入（网页账号）', gid: '', known: true };
      }
      refreshJoinedSoon();
  ...
  // scoped 属性从**已加载的官方 CSS**里反查（不写死 hash，官方改版也能跟上）。
  function scopeHashFor(cls) {
    try {
      ...
      for (var i = 0; i < sheets.length; i++) {
        ...
        for (var j = 0; rules && j < rules.length; j++) {
          ...
          if (sel.indexOf(cls) === -1) continue;
          var m = /data-v-([0-9a-z]{6,})/.exec(sel);
          if (m) return 'data-v-' + m[1];
        }
      }
    } catch (e) { /* 忽略 */ }
    return '';
  ...
  var _ghCache = {};
  function buildGuildHeader(info, num) {
    var hash = scopeHashFor('guild-info__basic__name') || scopeHashFor('guild-info__basic');
    var box = el('div', { 'class': 'guild-info__basic txpd-guild-header' });
    var top = el('div', { 'class': 'guild-info__basic__top top_title' });
    top.appendChild(el('h1', { 'class': 'guild-info__basic__name top_title_name' }, info.name || num));
    box.appendChild(top);
    var stat = el('h4', { 'class': 'guild-info__basic__statistics' });
    var line = el('span', { 'class': 'txpd-gh-stat' }, '');
    stat.appendChild(line);
    box.appendChild(stat);
    // 套上官方 scoped 属性，样式才吃得上
    if (hash) {
      var nodes = [box].concat(Array.prototype.slice.call(box.querySelectorAll('*')));
      nodes.forEach(function (n) { n.setAttribute(hash, ''); });
    }
    return box;
  }
  function ensureGuildHeaderFallback() {
    if (_pageOpen) return;                       // 整页视图不参与
    var num = currentGuildNumber();
    if (!num) return;
    var gm = document.querySelector('.game-guild-main');
    if (!gm) return;
    var mine = gm.querySelector('.txpd-guild-header');
    var officialName = gm.querySelector('.guild-info__basic__name');
    if (officialName && (officialName.textContent || '').trim()) {
      if (mine && mine.parentNode) mine.parentNode.removeChild(mine);   // 官方自己渲染了 → 撤掉兜底
      return;
    }
    var hit = _ghCache[num];
    var fresh = hit && (Date.now() - hit.ts < (hit.data ? 300000 : 60000));
    if (hit && hit.data) {
      if (!mine) {
        mine = buildGuildHeader(hit.data, num);
        gm.insertBefore(mine, gm.firstChild);
      }
      var nm = mine.querySelector('.guild-info__basic__name');
      if (nm) nm.textContent = hit.data.name || num;
      var st = mine.querySelector('.txpd-gh-stat');
      if (st) {
        var parts = [];
        if (hit.data.member_count) parts.push(hit.data.member_count + ' 成员');
        if (hit.data.guild_number) parts.push('频道号: ' + hit.data.guild_number);
        if (hit.data.guild_type) parts.push(hit.data.guild_type);
        st.textContent = parts.join('　');
      }
      return;
    }
    if (fresh) return;                           // 拉过（成功但还没数据 / 失败冷却中）
    _ghCache[num] = { ts: Date.now(), data: null };
    // 频道 id 不在 CLI 频道列表里（比如网页账号加入、CLI 未加入）→ 用频道号反查一次
    var fetchBy = function (gid) {
      if (!gid) return;
      api('/cli', { method: 'POST', body: { action: 'guild-info', params: { guild_id: gid } } }).then(function (r) {
        if (!r.success) return;                  // 冷却 60s 后重试
        _ghCache[num] = { ts: Date.now(), data: ((r.data && r.data.data) || {}) };
        try { queueUiSync(); } catch (e) { /* 忽略 */ }
      }).catch(function () { /* 忽略 */ });
    };
    var g = currentGuild();
    if (g.id) fetchBy(g.id);
    else resolveGuildIdByNumber(num, fetchBy);
  }
  ...
  function collapsePublishEditor(container) {
    // 把我们追加的编辑器摘掉、官方子节点恢复显示（展开时只隐藏、没删）
    var targets = container ? [container] : publishAreaContainers();
    Array.prototype.forEach.call(targets, function (box) {
      if (!box) return;
      Array.prototype.forEach.call(Array.prototype.slice.call(box.children), function (ch) {
        if (ch.getAttribute && ch.getAttribute('data-txpd-hidden-editor') === '1') {
          if (ch.style) ch.style.removeProperty('display');
          ch.removeAttribute('data-txpd-hidden-editor');
        } else if (ch.getAttribute && ch.getAttribute('data-txpd-editor-added') === '1' && ch.parentNode) {
          ch.parentNode.removeChild(ch);   // 只摘我们自己插进去的节点
        }
      });
  ...
  // 「点了没反应 / 发不出去」。所以统一按「可见的优先」来选。
  function publishAreaContainers() {
    return Array.prototype.slice.call(document.querySelectorAll('.publish-editor-container'));
  }
  function visiblePublishContainer() {
    var all = publishAreaContainers();
    for (var i = 0; i < all.length; i++) {
  ...
  function expandPublishEditor(container) {
    ...
    var holder = el('div', { 'class': 'txpd-editor-holder' });
    holder.innerHTML = OFFICIAL_COMPOSER_HTML;
    while (holder.firstChild) {
      var ch = holder.firstChild;
      if (ch.nodeType === 1) ch.setAttribute('data-txpd-editor-added', '1');
    ...
    // 我们这份还展开着，表现出来就是「点了 ×，× 没了但发帖还是展开的」。
    var closeBtn = mine.querySelector('.txpd-pub-close');
    if (closeBtn) closeBtn.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopPropagation();
      collapsePublishEditor(container);
    });
    // Markdown 仅长贴支持且必须带标题。
    var longPost = false;
    var useMd = false;
    var chipLong = el('span', { 'class': 'txpd-pub-chip', title: '长文（长贴）：需要标题，最多 10000 字' }, '长文');
    var chipMd = el('span', { 'class': 'txpd-pub-chip', title: 'Markdown 富文本：仅长文支持，需要标题' }, 'Markdown');
    var titleInput = el('input', {
      'class': 'txpd-pub-title', type: 'text', maxlength: '64',
      placeholder: '标题（长文 / Markdown 必填）',
    });
    var titleRow = el('div', { 'class': 'txpd-pub-titlerow' });
    titleRow.style.cssText = 'display:none;padding:0 12px 6px;';
    titleInput.style.cssText = 'width:100%;box-sizing:border-box;border:1px solid rgba(219,220,224,.9);'
      + 'border-radius:8px;padding:7px 10px;font-size:13px;outline:none;font-family:inherit;';
    titleRow.appendChild(titleInput);
    function chipStyle(c, on) {
      c.style.cssText = 'font-size:12px;padding:2px 10px;border-radius:100px;cursor:pointer;user-select:none;'
        + 'border:1px solid ' + (on ? 'transparent' : 'rgba(219,220,224,.9)') + ';'
        + 'background:' + (on ? 'var(--brand_standard,#2b64f5)' : '#fff') + ';'
        + 'color:' + (on ? '#fff' : '#666') + ';';
    }
    function wordLimit() { return longPost ? 10000 : 1000; }
    function refreshPubOpts() {
      chipStyle(chipLong, longPost);
      chipStyle(chipMd, useMd);
      titleRow.style.display = (longPost || useMd) ? 'block' : 'none';
      var lim = wordLimit();
      if (wordCount && contentEl) {
        var len = (contentEl.textContent || '').length;
        wordCount.textContent = Math.min(len, lim) + '/' + lim;
      }
    }
    chipLong.addEventListener('click', function (e) {
      e.stopPropagation();
      longPost = !longPost;
      if (!longPost) { useMd = false; }        // Markdown 依赖长文
      refreshPubOpts();
      updatePubState();
    });
    chipMd.addEventListener('click', function (e) {
      e.stopPropagation();
      useMd = !useMd;
      if (useMd) { longPost = true; if (!titleInput.value.trim()) titleInput.focus(); }
      refreshPubOpts();
      updatePubState();
    });
    titleInput.addEventListener('input', function () { updatePubState(); });
    // 放进我们的编辑器：标题在正文上方，两个开关放底栏（字数旁边）
    var areaRoot = mine.querySelector('.editor-area') || mine;
    var divider0 = areaRoot.querySelector('.editor-divider');
    if (divider0 && divider0.parentNode) divider0.parentNode.insertBefore(titleRow, divider0.nextSibling);
    else areaRoot.appendChild(titleRow);
    var bottomBar = areaRoot.querySelector('.bottom-bar');
    if (bottomBar) {
      var chipWrap = el('div', { 'class': 'txpd-pub-chips' });
      chipWrap.style.cssText = 'display:flex;gap:6px;align-items:center;margin-right:auto;order:-1;';
      chipWrap.appendChild(chipLong);
      chipWrap.appendChild(chipMd);
      bottomBar.insertBefore(chipWrap, bottomBar.firstChild);
    }
    refreshPubOpts();
    ...
    // 提交时由 serializeComposerContent 还原成官方节点语法
    function insertPubEmoji(ce, item) {
      if (!ce) return;
      ce.focus();
      if (String(item[2]) === '2') {
        insertNodeAtCaret(ce, document.createTextNode(String(item[0])));
      } else {
        insertNodeAtCaret(ce, el('img', {
          src: emojiImgUrl(item[0], item[2]),
          'data-emj': emojiInsertText(item),
          title: item[1],
      ...
      var area = ce.closest ? ce.closest('.editor-area') : null;
      var wc = area ? area.querySelector('.word-count') : null;
      if (wc) wc.textContent = Math.min((ce.textContent || '').length, 1000) + '/1000';
      if (ce === contentEl) updatePubState();
    }
    // 给「收起状态点表情图标」用：展开函数外也能插入表情
    container._txpdInsertEmoji = insertPubEmoji;
    // 官方改版时样式跟着走（采不到就保留内置属性）
    applyOfficialScopedAttrs(mine);
    if (contentEl) {
      ...
      contentEl.addEventListener('input', function () {
        var len = (contentEl.textContent || '').length;
        var lim = wordLimit();
        if (wordCount) wordCount.textContent = Math.min(len, lim) + '/' + lim;
        updatePubState();
      });
    }
    ...
    fileInput.setAttribute('data-txpd-editor-added', '1');   // 收起时一起摘掉，别留残留
    ...
    // 官方处理器（访客态下它只会弹登录/加入引导，什么都不显示）。
    if (container.getAttribute('data-txpd-emoji-wired') !== '1') {
      container.setAttribute('data-txpd-emoji-wired', '1');
      container.addEventListener('click', function (e) {
        var ic = e.target && e.target.closest ? e.target.closest('.toolbar-button .icon-emoji') : null;
        if (!ic) return;
        e.preventDefault();
        e.stopImmediatePropagation();
        e.stopPropagation();
    ...
    // 工具栏图片图标也触发选图（同样委托）
    if (container.getAttribute('data-txpd-image-wired') !== '1') {
      container.setAttribute('data-txpd-image-wired', '1');
      container.addEventListener('click', function (e) {
        var ic = e.target && e.target.closest ? e.target.closest('.toolbar-button .icon-image') : null;
        if (!ic) return;
        e.preventDefault();
        e.stopImmediatePropagation();
        e.stopPropagation();
    ...
    if (pubBtn) pubBtn.addEventListener('click', function (e) {
      ...
      // 没选版块时用该频道的默认（第一个）版块，并在状态行说明发到哪儿。
      var chId = chosen ? String(chosen.channel_id || '') : (channels[0] ? String(channels[0].channel_id || '') : '');
      if (!chId) {
        panelError(status, channelsLoaded ? '这个频道没有可用的版块，暂时发不了' : '版块列表还在加载，稍等一下再发（或点版块按钮里的「重新加载」）');
        loadChannels(true).then(function () { channelsErr = ''; });
        return;
      }
      status.style.color = '#888';
      status.textContent = '发到「' + (chosen ? (chosen.channel_name || chosen.channel_id) : (channels[0].channel_name || channels[0].channel_id)) + '」';
      // 长文 / Markdown 都必须有标题（CLI 会直接拦，这里提前说清楚）
      if ((longPost || useMd) && !titleVal) { titleInput.focus(); panelError(status, '长文 / Markdown 需要填写标题'); return; }
      if (content.length > wordLimit()) {
        panelError(status, (longPost ? '长文最多 10000 字' : '短贴最多 1000 字，长文请点「长文」'));
        return;
      }
      ...
      var doPublish = function (imagePaths) {
        ...
        api('/publish', {
          ...
          body: {
            ...
            content: content,
            images: imagePaths,
            format: useMd ? 'md' : 'text',
            feed_type: longPost ? '2' : '',
            user: TXPD_USER,
          },
        }).then(function (r) {
          if (!r.success) throw new Error(r.message || '发表失败');
          setStatus('✓ 已发表', '#15a361');
          contentEl.innerHTML = '';
          _pubImages = []; chosen = null; renderThumbs();
          titleInput.value = '';
          longPost = false; useMd = false; refreshPubOpts();
          if (wordCount) wordCount.textContent = '0/' + wordLimit();
          for (var i = 0; i < choseBtn.childNodes.length; i++) {
            var n = choseBtn.childNodes[i];
            if (n.nodeType === 3 && n.textContent.trim()) { n.textContent = ' 不选择版块'; break; }
          }
          updatePubState();
          setTimeout(function () { if (status.textContent === '✓ 已发表') setStatus(''); }, 2500);
        }).catch(function (e) { panelError(status, e.message); pubBtn.disabled = false; });
      };
      if (!_pubImages.length) { doPublish([]); return; }
      setStatus('正在上传图片 0/' + _pubImages.length + '…');
  ...
  // 采不到（页面还没渲染出官方节点）→ 保留内置属性，行为不变。
  var _scopeMap = null;
  function harvestScopeMap() {
    if (_scopeMap) return _scopeMap;
    var sample = document.querySelector('.game-guild-main__short-content')
      || document.querySelector('.game-guild-main__content')
      || document.querySelector('.game-guild-main');
    if (!sample) return null;                 // 还没渲染出来：下次再试
    var map = {};
    var nodes = [sample].concat(Array.prototype.slice.call(sample.querySelectorAll('*')));
    nodes.forEach(function (n) {
      if (!n.getAttribute) return;
      var cls = (n.getAttribute('class') || '').trim().split(/\s+/).filter(Boolean);
      if (!cls.length) return;
      Array.prototype.forEach.call(n.attributes || [], function (at) {
        if (at.name.indexOf('data-v-') !== 0) return;
        cls.forEach(function (one) {          // 每个 class 词都建索引：节点常带多个 class
          if (!map[one]) map[one] = {};
          map[one][at.name] = at.value || '';
        });
      });
    });
    _scopeMap = map;
  ...
  // 把我们节点上缺失的官方 scoped 属性补齐（已有的保留，互不影响）
  function applyOfficialScopedAttrs(root) {
    try {
      var map = harvestScopeMap();
      if (!map || !root) return;
      var nodes = [root].concat(Array.prototype.slice.call(root.querySelectorAll ? root.querySelectorAll('*') : []));
      nodes.forEach(function (n) {
        var cls = (n.getAttribute && n.getAttribute('class') || '').trim().split(/\s+/).filter(Boolean);
        if (!cls.length) return;
        cls.forEach(function (one) {
          if (!map[one]) return;
          Object.keys(map[one]).forEach(function (at) {
            if (!n.hasAttribute(at)) n.setAttribute(at, map[one][at]);
          });
        });
      });
    } catch (e) { /* 采集失败不影响渲染 */ }
  ...
  // opt.isReply=true → 楼中楼的回复：官方用 margin-left:38px + __child-avatar 表示层级
  function cliCommentRow(c, ctx, feed, onChanged, opt) {
    opt = opt || {};
    var isReply = !!opt.isReply;
    var nick = c.author || c.nickname || c.author_nick || '用户';
    var cid = c.comment_id || opt.parentCommentId || '';
    var rid = c.reply_id || '';
    var row = el('div', { 'class': 'comment-list-item', id: rid || cid });
    row.setAttribute('data-v-76bde4c4', '');
    row.setAttribute('data-v-2cbccead', '');
    row.setAttribute('data-txpd-cli-comment', '1');
    if (isReply) {
      row.setAttribute('data-txpd-cli-reply', '1');
      row.setAttribute('style', 'margin-left:38px;');   // 官方楼中楼缩进
    }
    row.innerHTML = '<div data-v-76bde4c4="" class="comment-list-item__info">'
      ...
      + '<div data-v-76bde4c4="" class="comment-list-item__info__title-reply-target" style="display:none;"><span class="triangle"></span><span class="txpd-reply-target-nick"></span></div>'
      ...
      + '</div></div></div>';
    row.querySelector('.txpd-av-slot').appendChild(avatarBox(30, isReply ? 'comment-list-item__child-avatar' : 'comment-list-item__avatar', nick));
    row.querySelector('.comment-list-item__info__title-name').textContent = nick;
    // 楼中楼回复的「回复 @xxx」标记（CLI 的 target 字段；内容里的 @ 节点也保留）
    var targetNick = c.target_user_nick || c.targetUserNick || c.reply_to_nick || c.replyToNick || '';
    if (targetNick) {
      var tEl = row.querySelector('.comment-list-item__info__title-reply-target');
      tEl.style.display = '';
      tEl.querySelector('.txpd-reply-target-nick').textContent = targetNick;
    }
    ...
    // 点赞：回复用 reply_id + reply_author_id 走同一条 do-like
    var likeKey = rid || cid;
    var liked0 = !!likedCommentsSet()[likeKey];
    ...
    likeEl.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopPropagation();
      var liked = !!likedCommentsSet()[likeKey];
      toast(liked ? '取消点赞中…' : '点赞中…');
      var params = {
        ...
        comment_create_time: String(c.create_time_raw || c.create_time || opt.commentCreateTime || ''),
        like_type: liked ? '4' : '3',
        guild_id: ctx.guildId, channel_id: ctx.channelId,
      };
      if (rid) {
        params.reply_id = rid;
        params.reply_author_id = c.author_id || '';
      }
      api('/cli', { method: 'POST', body: { action: 'like-comment', params: params } }).then(function (r) {
        if (!r.success) throw new Error(r.message || '操作失败');
        var s2 = likedCommentsSet();
        if (liked) delete s2[likeKey]; else s2[likeKey] = 1;
        saveLikedComments(s2);
    ...
    row.querySelector('.comment-list-item__rely').addEventListener('click', function (e) {
      ...
      if (!ed || !ed._setMode) return;
      // 回复一条回复 → 楼中楼：带上 target_* 让上游在内容前插入「回复 @昵称」
      if (isReply && rid) {
        ed._setMode('reply', cid, nick, c, {
          targetReplyId: rid,
          targetUserId: c.author_id || '',
          targetUserNick: nick,
          commentCreateTime: String(c.create_time_raw || c.create_time || opt.commentCreateTime || ''),
        });
      } else {
        ed._setMode('reply', cid, nick, c);
      }
  ...
  // 评论的楼中楼回复列表（含「查看更多回复」分页）
  function cliReplyBlock(c, ctx, feed, onChanged) {
    var host = el('div', { 'class': 'txpd-reply-block' });
    host.setAttribute('data-comment-id', String(c.comment_id || ''));
    function replyList() {
      var list = c.reply_list || c.replyList || c.replies || c.reply_items;
      return Array.isArray(list) ? list : [];
    }
    function replyCount() {
      var n = Number(c.reply_count || c.replyCount || c.reply_items_count || 0);
      var shown = replyList().length;
      return n > shown ? n : shown;
    }
    var more = el('div', { 'class': 'txpd-more-replies pointer' });
    more.style.cssText = 'margin-left:38px;padding:2px 0 2px;font-size:13px;color:#2b64f5;';
    function paint() {
      var list = replyList();
      Array.prototype.forEach.call(host.querySelectorAll('.txpd-reply-row'), function (n) { n.parentNode.removeChild(n); });
      list.forEach(function (rp) {
        var row = cliCommentRow(rp, ctx, feed, onChanged, { isReply: true, parentCommentId: c.comment_id, commentCreateTime: String(c.create_time_raw || c.create_time || '') });
        row.classList.add('txpd-reply-row');
        host.insertBefore(row, more);
      });
      var total = replyCount();
      var attach = c.attach_info || c.attachInfo || '';
      var hasMore = c.has_more_replies === true || c.hasMoreReplies === true || !!attach;
      if (!hasMore) { more.style.display = 'none'; return; }
      more.style.display = '';
      more.textContent = list.length
        ? ('查看更多回复（还有 ' + Math.max(0, total - list.length) + ' 条）')
        : ('展开 ' + (total || '') + ' 条回复');
    }
    more.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopPropagation();
      if (more.getAttribute('data-loading') === '1') return;
      more.setAttribute('data-loading', '1');
      var old = more.textContent;
      more.textContent = '加载中…';
      api('/cli', { method: 'POST', body: { action: 'replies', params: {
        feed_id: ctx.feedId, comment_id: c.comment_id, guild_id: ctx.guildId, channel_id: ctx.channelId,
        count: '20', attach_info: c.attach_info || c.attachInfo || '',
      } } }).then(function (rr) {
        if (!rr.success) throw new Error(rr.message || '加载回复失败');
        var d = (rr.data && rr.data.data) || {};
        var items = d.replies || d.items || d.list || [];
        c.reply_list = replyList().concat(items);
        c.attach_info = d.attach_info || d.next_page_cookie || '';
        c.has_more_replies = !!c.attach_info;
        c.reply_count = Math.max(Number(c.reply_count || 0), c.reply_list.length);
        more.removeAttribute('data-loading');
        more.textContent = old;
        paint();
        if (!items.length) toast('没有更多回复了');
      }).catch(function (err) {
        more.removeAttribute('data-loading');
        more.textContent = old;
        toast('加载回复失败：' + String((err && err.message) || err).slice(0, 50));
      });
    });
  ...
  function buildCliEditor(ctx, onSent) {
    ...
    var mode = 'comment', cid = '', cobj = null, extra = null;
    function grow() { /* contenteditable 自适应高度 */ }
    function setMode(m, id, nick, c, opt) {
      mode = m || 'comment';
      cid = id || '';
      cobj = c || null;
      extra = opt || null;   // 楼中楼：{ targetReplyId, targetUserId, targetUserNick }
      cancel.style.display = (mode === 'reply') ? 'inline-flex' : 'none';
      var who = (extra && extra.targetUserNick) || nick;
      ta.setAttribute('data-ph', mode === 'reply' ? ('回复 ' + (who || '') + '…') : '说点什么…');
    ...
    send.addEventListener('click', function (e) {
      ...
      submitComment(mode, cid, content, {
        ...
        commentCreateTime: (cobj ? String(cobj.create_time_raw || cobj.create_time) : '') || (extra ? extra.commentCreateTime : ''),
        targetReplyId: extra ? extra.targetReplyId : '',
        targetUserId: extra ? extra.targetUserId : '',
        targetUserNick: extra ? extra.targetUserNick : '',
      }).then(function () {
        ta._imgPath = '';
  ...
  // ---------- 帖子详情缓存（点开秒出）：命中 60 秒内的结果直接渲染 ----------
  var _feedDetailCache = {};
  var _feedDetailInflight = {};
  function fetchFeedDetail(feedId, guildId, channelId) {
    feedId = String(feedId || '');
    var hit = _feedDetailCache[feedId];
    if (hit && Date.now() - hit.ts < 60000) return Promise.resolve(hit.payload);
    if (_feedDetailInflight[feedId]) return _feedDetailInflight[feedId];
    var pr = api('/cli', { method: 'POST', body: { action: 'feed-detail', params: { feed_id: feedId, guild_id: guildId || '', channel_id: channelId || '' } } }).then(function (r) {
      delete _feedDetailInflight[feedId];
      if (r && r.success) _feedDetailCache[feedId] = { ts: Date.now(), payload: r };
      return r;
    }).catch(function (e) {
      delete _feedDetailInflight[feedId];
      throw e;
    });
    _feedDetailInflight[feedId] = pr;
    return pr;
  }
  // 悬停/按下时先把详情拉回来（真正点开时不用等）
  function prefetchFeedDetail(feedId, guildId) {
    feedId = String(feedId || '');
    if (!feedId) return;
    var hit = _feedDetailCache[feedId];
    if (hit && Date.now() - hit.ts < 60000) return;
    fetchFeedDetail(feedId, guildId || '', '').catch(function () { /* 预取失败不影响点击 */ });
  }

  function renderCliFeedDetail(body, feedId, guildId, channelId, fallbackUrl) {
    body.setAttribute('data-feed', String(feedId || ''));
    body.innerHTML = '';
    var wrap = el('div', { 'class': 'txpd-cli-detail' });
    body.appendChild(wrap);
    wrap.appendChild(el('div', { 'class': 'txpd-cli-empty' }, '正在通过插件账号加载帖子…'));
    fetchFeedDetail(feedId, guildId, channelId).then(function (r) {
      ...
      // CLI 拿不到内容（权限/风控等）→ 回退官方 iframe（有网页登录态时仍能看）
      var ctx = {
        feedId: feedId,
      ...
      function loadComments() {
        ...
        // 以前没传这个参数，前端也没画 reply_list，所以「评论的评论」根本显示不出来。
        api('/cli', { method: 'POST', body: { action: 'comments', params: { feed_id: feedId, guild_id: ctx.guildId, channel_id: ctx.channelId, count: '20', reply_list_num: '10' } } }).then(function (rc) {
          ...
          list.forEach(function (c) {
            clist.appendChild(cliCommentRow(c, ctx, feed, loadComments));
            var replies = c.reply_list || c.replyList || c.replies || c.reply_items;
            var count = Number(c.reply_count || c.replyCount || (Array.isArray(replies) ? replies.length : 0));
            if (count > 0) clist.appendChild(cliReplyBlock(c, ctx, feed, loadComments));
          });
        }).catch(function () {
          clist.innerHTML = '';
      ...
      wrap.innerHTML = '';
      if (fallbackUrl) { openEmbedOverlay(fallbackUrl, '帖子详情'); return; }   // CLI 拿不到就回退官方 iframe
  ...
  function dynItemEl(f) {
    ...
    // （保留 href，中键/新窗口打开等默认行为不受影响；卡片内图片、点赞等已各自 stopPropagation）
    a.addEventListener('click', function (e) {
      if (e.defaultPrevented || e.button !== 0 || e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;
      e.preventDefault();
      spaNavigate(String(a.getAttribute('href') || '').slice(PANEL_BASE.length));
    });
    a.innerHTML = ''
  ...
  function openCliDetailOverlay(feedId, guildId, title, fallbackUrl) {
    var ov = ensureOverlayShell();
    ...
    ov._title.textContent = title || '帖子详情';
    ov.style.display = 'flex';
    renderCliFeedDetail(ov._dom, feedId, guildId, '', fallbackUrl);
    return true;
  }
  // 调试钩子：按 feed_id 直接打开 CLI 详情浮层（排查帖子详情 / 楼中楼评论渲染时用）
  window.__txpdOpenDetail = function (feedId, guildId, title) {
    return openCliDetailOverlay(feedId, guildId, title);
  };
  ...
  function submitComment(mode, commentId, content, opt) {
    ...
    return prep.then(function () {
      // 楼中楼：回复对象是「某条回复」时，CLI 需要 target_* 才会在内容前插入「回复 @昵称」节点
      var targets = {};
      if (opt && opt.targetReplyId && opt.targetUserId) {
        targets = {
          target_reply_id: opt.targetReplyId,
          target_user_id: opt.targetUserId,
          target_user_nick: opt.targetUserNick || '',
        };
      }
      if (mode !== 'reply') {
      ...
      var base = { feed_id: ctx.feedId, guild_id: ctx.guildId, channel_id: ctx.channelId, feed_create_time: ctx.feedCreateTime, comment_id: commentId, content: content, reply_type: '1' };
      Object.keys(targets).forEach(function (k) { base[k] = targets[k]; });
      if (opt && opt.commentAuthorId) {
        base.comment_author_id = opt.commentAuthorId;
        base.comment_create_time = opt.commentCreateTime || '';
        return api('/cli', { method: 'POST', body: { action: 'do-reply', params: base } });
      }
      return api('/cli', { method: 'POST', body: { action: 'comments', params: { feed_id: ctx.feedId } } }).then(function (rc) {
        var comments = (rc.data && rc.data.data && rc.data.data.comments) || [];
        var c = null;
        ...
        if (!rc.success || !c) throw new Error('未找到该评论，请刷新后重试');
        base.comment_author_id = c.author_id;
        base.comment_create_time = String(c.create_time_raw || c.create_time);
        return api('/cli', { method: 'POST', body: { action: 'do-reply', params: base } });
      });
    }).then(function (r) {
  ...
  function queueUiSync() {
    ...
    setTimeout(function () {
      ...
      ensureNavEntries();
      syncPublishArea();
      refreshJoinedAvatars();   // 官方「我的频道」晚一步渲染：每轮回来把头补齐
      syncAsideTitles();        // 官方「我的频道」标题按来源改名（会被应用重渲染还原）
      ensureMobileBackButton();
      ensureNarrowDrawer();
      syncLikedMarks();
      ensureGuildMeta();
      syncCommentPlaceholder();
      syncSsrExploreVisibility();
      ensureDynamicPage();
  ...
  function mount() {
    ...
    window.addEventListener('click', function (e) {
      ...
      if (opBtn) {
        ...
        else if (opKind === 'leave') openLeaveGuild(opBtn);
        return;
      }
      ...
      // 鼠标扫过帖子卡片就先取详情（点开时不用等 CLI）；capture 阶段，避免被应用吞掉
      if (!TXPD_EMBED) {
        var preT = null;
        window.addEventListener('mouseover', function (e) {
          var a = e.target && e.target.closest ? e.target.closest('a[href*="/post/"]') : null;
          if (!a) return;
          if (a.getAttribute('data-txpd-cli-detail') === '1') { prefetchFeedDetail(a.getAttribute('data-feed'), a.getAttribute('data-guild')); return; }
          var m2 = /\/g\/([^\/?#]+)\/post\/([^\/?#]+)/.exec(a.getAttribute('href') || '');
          if (!m2) return;
          if (preT) clearTimeout(preT);
          preT = setTimeout(function () { prefetchFeedDetail(decodeURIComponent(m2[2]), ''); }, 120);
        }, true);
      }
      ...
      if (!TXPD_EMBED) {
        ...
        if (postA && !(postA.closest && postA.closest('#txpd-pop'))) {
          var im = /\/g\/([^\/?#]+)\/post\/([^\/?#]+)/.exec(postA.getAttribute('href') || '');
          var opened = false;
          if (postA.getAttribute('data-txpd-cli-detail') === '1') {
            opened = openCliDetailOverlay(postA.getAttribute('data-feed'), postA.getAttribute('data-guild'), '帖子详情');
          } else if (im) {
            // 直接用 CLI 详情浮层；CLI 拉不到再回退 iframe。
            var pNum = decodeURIComponent(im[1]);
            var pFeed = decodeURIComponent(im[2]);
            var pGid = (currentGuild().num === pNum ? currentGuild().id : '') || _gidByNumber[pNum] || '';
            opened = openCliDetailOverlay(pFeed, pGid, '帖子详情', postA.getAttribute('href') || '');
            if (opened && !pGid) {
              resolveGuildIdByNumber(pNum, function (gid2) {
                if (!gid2) return;
                var ov0 = document.getElementById('txpd-embed-overlay');
                var body0 = document.getElementById('txpd-embed-dom');
                if (ov0 && body0 && ov0.style.display === 'flex' && body0.getAttribute('data-feed') === String(pFeed)) {
                  renderCliFeedDetail(body0, pFeed, gid2, '');
                }
              });
            }
          } else {
            opened = openPostOverlay(postA.getAttribute('href') || '');
          }
          if (opened) {
            e.preventDefault();
            e.stopImmediatePropagation();
