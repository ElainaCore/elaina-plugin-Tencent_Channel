...
def _is_comment_notice(item: Dict[str, Any]) -> bool:
    ...
    kind = str(
        item.get("type") or item.get("notice_type") or item.get("noticeType") or ""
    ).lower()
    if kind:
...
def _fetch_feed_context(feed_id: str, guild_id: str, user: str) -> Dict[str, Any]:
    ...
    if ok:
        ...
        feed_obj = (
            payload.get("feed") if isinstance(payload.get("feed"), dict) else payload
        )
        if isinstance(feed_obj, dict):
            ctx["title"] = (
                _text_of(feed_obj.get("title"))
                or _text_of(feed_obj.get("content"))[:30]
            )
            ctx["feed_author_id"] = str(
                feed_obj.get("author_id")
                or feed_obj.get("authorId")
                or feed_obj.get("feed_author_id")
                or ""
            ).strip()
            ctx["feed_create_time"] = str(
                feed_obj.get("create_time_raw")
            ...
            ctx["channel_id"] = str(
                feed_obj.get("channel_id") or feed_obj.get("channelId") or ""
            ).strip()
...
def _iter_targets(ctx: Dict[str, Any]):
    ...
    for comment in ctx["comments"]:
        ...
        comment_ts = str(
            comment.get("comment_create_time")
            or comment.get("create_time_raw")
        ...
        if not (cid and comment_author and comment_ts):
            continue
        yield (
            _create_time(comment),
            {**base, "nick": _item_nick(comment), "content": _comment_content(comment)},
        )
        replies = (
            comment.get("replies")
            or comment.get("replies_preview")
            or comment.get("reply_list")
            or comment.get("replyList")
        )
...
def _target_from_item(
    ...
    if comment_id:
        candidates = [
            t
            for _, t in _iter_targets(ctx)
            if t.get("comment_id") == comment_id and not t.get("target_reply_id")
        ]
        if candidates:
            return candidates[-1]
    ...
    if summary:
        matched = [
            (ts, t)
            for ts, t in _iter_targets(ctx)
            if t.get("content", "").strip() == summary
        ]
        if matched:
    ...
    for comment in ctx["comments"][:20]:
        ...
        if isinstance(replies, list):
            for reply in replies[:5]:
                ...
                reply_nick = _item_nick(reply) or "未知用户"
                lines.append(
                    f"　↳ {reply_nick}：{_comment_content(reply)[:NOTIFY_TEXT_LIMIT]}"
                )
    return "\n".join(lines)
...
async def _notify_admins(
    ...
    fallback = (
        _comments_as_text(ctx) if (with_image and not image and ctx["comments"]) else ""
    )
...
async def _poll_slot(user: str) -> None:
    ...
    for _, item in fresh:
        ...
        if target and not target.get("nick"):
            author = (
                target.get("target_user_id") or target.get("comment_author_id") or ""
            )
            target["nick"] = await asyncio.to_thread(
                _lookup_nick, user, str(ctx.get("guild_id") or ""), str(author)
            )
        seq = _push_ctx(
            {**(target or {"feed_id": feed_id}), "kind": "comment", "user": user}
        )
        # 同一帖子在一轮轮询里只发一次评论列表图片，避免重复
        await _notify_admins(
            user, item, ctx, target, seq, with_image=feed_id not in imaged_feeds
        )
...
def _is_dm_notice(item: Dict[str, Any]) -> bool:
    ...
    if "dm" in kind or "private" in kind or "私信" in kind:
        return True
    return bool(
        item.get("peer_tiny_id") or item.get("peerTinyId") or item.get("from_tiny_id")
    )


def _dm_fields(item: Dict[str, Any]) -> Dict[str, str]:
    peer = str(
        item.get("peer_tiny_id")
        or item.get("peerTinyId")
        or item.get("from_tiny_id")
        or item.get("tiny_id")
        or item.get("poster_tiny_id")
        or item.get("sender_tiny_id")
        or ""
    ).strip()
            ...
            if isinstance(members, list):
                for member in members:
                    if (
                        ...
                        and str(member.get("tinyid") or "").strip() == from_id
                    ):
...
async def _queue_dm_notice(
    user: str, fields: Dict[str, str], nick: str, content: str
) -> None:
...
async def _poll_dm_slot(user: str) -> None:
    ...
    ok, output, stderr = await asyncio.to_thread(
        _run_cli_full, ["manage", "check-notices", "--json"], None, user or None
    )
    if not ok:
        return
    payload = _payload_of(output)
    ...
    if dm_msgs:
        self_id = str(
            await asyncio.to_thread(_get_self_user_id, None, user or None) or ""
        ).strip()
    for _, item in fresh:
...
async def handle_notify_toggle(event, match):
    ...
    await event.reply(
        f"✅ 评论提醒已{'开启' if enabled else '关闭'}（每 {notify_poll_interval()} 秒轮询各账号槽位的互动消息）"
    )
...
async def handle_dm_notify_toggle(event, match):
    ...
    await event.reply(
        f"✅ 私信提醒已{'开启' if enabled else '关闭'}（每 {notify_poll_interval()} 秒轮询各账号槽位的私信通知）"
    )
...
async def handle_poll_interval(event, match):
    ...
    if not raw:
        await event.reply(
            f"当前通知轮询间隔：{notify_poll_interval()} 秒（发送「通知间隔 秒数」修改，范围 {POLL_INTERVAL_MIN}-{POLL_INTERVAL_MAX}）"
        )
        return
    value = max(POLL_INTERVAL_MIN, min(int(raw), POLL_INTERVAL_MAX))
    _set_setting("notify_poll_interval", value)
    await event.reply(
        f"✅ 通知轮询间隔已设为 {value} 秒（提醒最迟约 间隔+私信冷却 秒后送达）"
    )
...
async def handle_dm_merge_window(event, match):
    ...
    if not raw:
        await event.reply(
            f"当前私信合并冷却：{dm_merge_window()} 秒（发送「私信冷却 秒数」修改，0 为不合并，最大 {DM_MERGE_WINDOW_MAX}）"
        )
        return
...
async def handle_quick_reply(event, match):
    ...
    args = [
        ...
        ctx["feed_author_id"],
        "--feed-create-time",
        ctx["feed_create_time"],
        "--comment-author-id",
        ctx["comment_author_id"],
        "--comment-create-time",
        ctx["comment_create_time"],
        "--content",
        content,
        "--json",
    ]
